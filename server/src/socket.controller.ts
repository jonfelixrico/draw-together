...
export function socketIOHandler (io: Server) {
  ...
  io.on('connect', (socket: Socket) => {
    ...
    socket.join(roomId)
    socket.broadcast.to(roomId).emit('PAD_EVENT', {
      code: 'CONN_ACTIVITY',
      payload: {
        id: clientId,
        name,        
    ...
    socket.on('PAD_EVENT', (payload: Record<string, unknown>) => {
      console.debug('Relayed %o to room %s, from client %s', payload, roomId, clientId)
      socket.broadcast.to(roomId).emit('PAD_EVENT', payload)
    })
  })
}